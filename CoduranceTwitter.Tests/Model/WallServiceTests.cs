...
namespace CoduranceTwitter.Tests.Model
{
    ...
    public class WallTests
    {
        ...
        public void Read_FollowingOneUser()
        {
            ...
            Message message2 = new Message() { Username = new User(TEST_USER_FOLLOW), Text = TEST_MESSAGE_FOLLOW, Timespan = dt.AddHours(1)};

            messageRepository.Add(message1);
            messageRepository.Add(message2);
            var subscription = new Wall() { Username = new User(TEST_USER), FollowUser = new User(TEST_USER_FOLLOW) };
